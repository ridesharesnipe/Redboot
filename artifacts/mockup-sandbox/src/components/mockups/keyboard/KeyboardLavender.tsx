...
export function KeyboardLavender() {
  ...
  return (
    <div style={{
      ...
      {/* KEYBOARD — soft candy lavender */}
      <div style={{
        position: "absolute", bottom: 0, left: 0, right: 0,
        background: "linear-gradient(180deg, #EDE9FE 0%, #DDD6FE 60%, #C4B5FD 100%)",
        borderTop: "2px solid rgba(167,139,250,0.6)",
        borderRadius: "24px 24px 0 0",
        padding: "14px 4px 20px",
        boxShadow: "0 -10px 36px rgba(139,92,246,0.18)",
      }}>
        <div style={{ width: 36, height: 4, borderRadius: 2, background: "rgba(139,92,246,0.3)", margin: "0 auto 12px" }}/>
        {QWERTY_ROWS.map((row, ri) => (
          <div key={ri} style={{ display: "flex", justifyContent: "center", gap: 0, marginBottom: 0 }}>
            {row.map(key => (
              <button key={key} onClick={() => handleKey(key)} className="clay-key-lav">{key}</button>
            ))}
          </div>
        ))}
      </div>
      ...
      <style>{`
        ...
        .clay-key-lav:active { transform: scale(0.94) translateY(1px); }
      `}</style>
    </div>
  );
}
